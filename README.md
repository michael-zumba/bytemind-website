...
*   `bytebook/`: ByteBook product page, and the training handbook the site publishes (`bytebook/manual/`). The handbook is generated: chapters are written in the ByteBook project at `ByteBook/manual/content/` and built with `python manual/build_manual.py`, which checks every standard reference against the publisher's own text before writing the reader. The reader lays each chapter out as pages one screen wide and turns them with a page-flip animation; arrow keys, the buttons or a swipe move through the book, and the end of a chapter leads into the next one. The product page embeds the book in an iframe and lists the chapters from `bytebook/manual/contents.json`, so the two cannot drift; `?embed=1` on any handbook page drops the reader's own bar and contents rail for use inside a frame. The short clips on the product page are recorded from the running app with `ByteBook/scripts/record_demos.py`, so a clip cannot show something the software does not do.
